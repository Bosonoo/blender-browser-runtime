...
def validate(operation, args):
    ...
    if operation == "blender.scene.edit":
        ...
        for op in ops:
            ...
            elif op.get("kind") == "set_material":
                _fields(op, ("kind", "object_name", "color"), ("kind", "object_name", "color"))
                _name(op["object_name"])
            else:
        ...
        _fields(args, ("expected_scene_revision", "name"), ("expected_scene_revision", "name"))
        _name(args["name"])
    else:
...
def _material(bpy, name, color, image=None):
...
def execute(bpy, operation, args, command_id):
    ...
    if operation == "blender.scene.export_glb":
        ...
        result = bpy.ops.export_scene.gltf(filepath=export_path, check_existing=False, export_format="GLB",
    ...
    created_objects, created_meshes, materials, backups, imported = [], [], [], [], None
    try:
        _busy = True
        if operation == "blender.scene.edit":
            ...
            for op in args["operations"]:
                if op["kind"] == "create_mesh":
                    ...
                    backups.append((obj, obj.location.copy(), obj.rotation_euler.copy(), obj.scale.copy(), list(obj.data.materials)))
            ...
            if target is not None:
                backups.append((target, target.location.copy(), target.rotation_euler.copy(), target.scale.copy(), list(target.data.materials)))
        ...
        return {"scene_revision": _revision, "changed": True, "created_objects": [obj.name for obj in created_objects],
                "image": imported.name if imported is not None else None, "persistence": "pending_native_save"}
    except Exception:
        for obj, location, rotation, scale, prior in reversed(backups):
            obj.location, obj.rotation_euler, obj.scale = location, rotation, scale
            obj.data.materials.clear()
            for material in prior:
                obj.data.materials.append(material)
